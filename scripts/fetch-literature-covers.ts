...
const novels = [
  ["literature-novel-animal-farm", "https://covers.openlibrary.org/b/isbn/9780451526342-L.jpg"],
  ["literature-novel-mistborn-era-1", "https://covers.openlibrary.org/b/isbn/9781250267177-L.jpg"],
] as const;
...
async function anilistCover(search: string) {
  ...
  console.log(`${search}: ${media.title.english ?? media.title.romaji ?? "matched"}`);
...
async function saveCover(workId: string, url: string) {
  ...
  if (bytes.length < 5_000) throw new Error(`Cover is unexpectedly small: ${url}`);
  ...
  const current = db
    ...
    .from(assets)
    .where(
      and(eq(assets.ownerType, "work"), eq(assets.ownerId, workId), eq(assets.assetType, "poster")),
    )
    .get();
  ...
  if (current) {
    ...
    db.insert(assets)
      .values({
        id: createHash("sha256").update(`asset:${workId}:poster`).digest("hex").slice(0, 32),
        ownerType: "work",
        ownerId: workId,
        assetType: "poster",
        relativePath,
