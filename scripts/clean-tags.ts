...
const runTagFormatter = db.transaction(() => {
  ...
  const tagTerms = db
    .prepare<[string], TermRow>(
      "SELECT id, vocabulary, name, slug FROM terms WHERE vocabulary = ?"
    )
    .all("tag")
  ...
  const updateTermStmt = db.prepare(
    "UPDATE terms SET name = ?, slug = ? WHERE id = ?"
  )
  const updateWorkTermStmt = db.prepare(
    "UPDATE OR IGNORE work_terms SET term_id = ? WHERE term_id = ?"
  )
  const deleteWorkTermStmt = db.prepare(
    "DELETE FROM work_terms WHERE term_id = ?"
  )
  ...
  for (const [newSlug, entries] of grouped.entries()) {
    ...
    if (
      primary.term.name !== primary.newName ||
      primary.term.slug !== newSlug
    ) {
