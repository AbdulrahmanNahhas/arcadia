...
const scalar = (query: string) => (sqlite.prepare(query).get() as { total: number }).total;
...
const report = {
  databasePath,
  worksByKind: sqlite
    .prepare("select kind, count(*) as total from works group by kind order by kind")
    .all(),
  personalStatus: sqlite
    .prepare("select status, count(*) as total from personal_state group by status order by status")
    .all(),
  ...
  checks: {
    ...
    orphanedContributions: scalar(
      "select count(*) as total from work_contributions left join works on work_contributions.work_id = works.id left join entities on work_contributions.entity_id = entities.id where works.id is null or entities.id is null",
    ),
    ...
    missingSummaries: scalar("select count(*) as total from works where trim(summary) = ''"),
    ...
    missingContributions: scalar(
      "select count(*) as total from works where not exists (select 1 from work_contributions where work_contributions.work_id = works.id)",
    ),
    missingLinks: scalar(
    ...
    tagVocabularySize: scalar("select count(*) as total from terms where vocabulary = 'tag'"),
