...
INSERT INTO `__new_history_events`("id", "work_id", "season_id", "unit_id", "related_event_id", "external_key", "import_batch_id", "event_type", "occurred_at", "recorded_at", "source", "status_before", "status_after", "progress_before", "progress_after", "rating_value", "duration_minutes", "notes")
SELECT "id", "work_id", "season_id", "unit_id", null, "external_key",
	"import_batch_id", "event_type", "occurred_at", "recorded_at", "source",
	"status_before", "status_after", "progress_before", "progress_after",
	"rating_value", "duration_minutes", "notes"
...
CREATE INDEX `history_events_related_event_idx` ON `history_events` (`related_event_id`);
--> statement-breakpoint
...
END;
--> statement-breakpoint
CREATE TRIGGER `history_events_validate_related`
BEFORE INSERT ON `history_events`
WHEN NEW.related_event_id IS NOT NULL
  AND NOT EXISTS (
    SELECT 1 FROM history_events
    WHERE id = NEW.related_event_id AND work_id = NEW.work_id
  )
BEGIN
  SELECT RAISE(ABORT, 'related event does not belong to work');
END;
