...
await page.getByPlaceholder("Name your view...").fill("Action library")
await page.getByRole("button", { name: "Save", exact: true }).click()
await page
  .locator('[data-sidebar="sidebar"]')
  .getByText("Action library")
  .waitFor()
...
if (!tableCover || tableCover.width < 38 || tableCover.height < 54) {
...
await page.screenshot({
  path: "/tmp/arcadia-timeline-dark.png",
  fullPage: false,
})
...
if (
  (await page.evaluate(() => localStorage.getItem("arcadia:sidebar-open"))) !==
  "false"
) {
  throw new Error("Sidebar state was not persisted")
}
await page.screenshot({
  path: "/tmp/arcadia-sidebar-collapsed.png",
  fullPage: false,
})
...
await page
  ...
  .waitFor()
await page.getByText("Progress ledger", { exact: true }).waitFor()
await page.getByRole("link", { name: /MyAnimeList/ }).waitFor()
await page.getByRole("button", { name: "Open full screen" }).click()
if (
  !(await page.getByRole("dialog").getAttribute("class"))?.includes(
    "full-screen"
  )
) {
