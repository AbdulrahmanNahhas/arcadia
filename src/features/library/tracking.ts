import type { TrackingEntry, Work, WorkSeasonDetail, WorkStructure } from "./model";
...
export function activityAmount(entry: Pick<TrackingEntry, "progressBefore" | "progress">) {
...
export function progressSegments(
  ...
  for (const season of structure?.seasons ?? []) {
    ...
    appendIntersection(segments, progressBefore, progressAfter, offset, capacity, season);
    offset += capacity;
  }

  const ungroupedCapacity = structure?.ungroupedUnits.length ?? 0;
  appendIntersection(segments, progressBefore, progressAfter, offset, ungroupedCapacity, null);
...
export function isMovieStatusEvent(
  ...
  return (
    work.kind === "movie" && entry.status === "completed" && entry.statusBefore !== "completed"
  );
}
