      ...
      {[...groups.entries()]
        ...
        .map(([year, items]) => (
          <section key={year} className="grid md:grid-cols-[110px_1fr]">
            ...
            <div className="relative grid gap-2 border-r pr-5 pb-8! before:absolute before:top-2 before:-right-1.25 before:size-2.5 before:rounded-full before:border-2 before:border-background before:bg-primary last:pb-0 lg:grid-cols-2">
              {items.map((work) => (
                <button
                  type="button"
                  key={work.id}
                  onClick={() => onOpen(work.id)}
                  className="group flex min-w-0 items-center gap-3 rounded-xl border bg-card p-2.5 text-right shadow-xs transition hover:border-primary/30 hover:bg-accent/25 hover:shadow-sm focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none"
                >
